...
class LoginVC: UIViewController {
    ...
    func handleRegister() {
        ...
        guard let number1 = phoneNumberTextField.text else { return }
        guard let email1 = emailTextField.text else { return }
        guard let password1 = passwordTextField.text else { return }
        
        
        /* PART 1B FINISH*/
        Auth.auth().createUser(withEmail: email1, password: password1, completion: { (user, error) in
            if let error = error {
                self.loginRegisterButton.isUserInteractionEnabled = true
                print(error)
                ...
                let values = ["name": username, "number": number1, "email": email1]
